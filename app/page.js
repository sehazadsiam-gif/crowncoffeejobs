...
export default function HomePage() {
  ...
  if (status === 'success') {
    return (
      <main style={{ minHeight: '100vh', minHeight: '100dvh', background: 'linear-gradient(160deg, #F5F0E8 0%, #EDE7DB 50%, #F5F0E8 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem' }}>
        <div className="success-box">
          <div style={{ position: 'relative', width: 90, height: 90, margin: '0 auto 1.75rem' }}>
            ...
            <div style={{ width: 90, height: 90, borderRadius: '50%', background: 'linear-gradient(135deg, rgba(200,146,42,0.15), rgba(200,146,42,0.05))', border: '2px solid var(--gold-soft)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 36, color: 'var(--gold-deep)', backdropFilter: 'blur(10px)' }}>✓</div>
          </div>
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(22px, 6vw, 32px)', fontWeight: 700, color: 'var(--charcoal)', marginBottom: 12 }}>{c.successTitle}</h1>
          <p style={{ color: 'var(--text-secondary)', fontSize: 'clamp(14px, 3.5vw, 16px)', marginBottom: '2.5rem', lineHeight: 1.7 }}>{c.successMsg}</p>
          <button onClick={resetForm} style={{ padding: '14px 36px', background: 'var(--charcoal)', color: 'var(--white)', border: 'none', borderRadius: 50, fontWeight: 600, fontSize: 14, fontFamily: 'var(--font-body)', cursor: 'pointer', transition: 'all 0.3s', letterSpacing: '0.5px', minHeight: 48 }}
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: 12, marginTop: '2.5rem' }}>01771784474 · Sector-13, Uttara</p>
        </div>
      </main>
    );
  }
  ...
  return (
    <main style={{ minHeight: '100vh', minHeight: '100dvh', background: 'linear-gradient(160deg, #F5F0E8 0%, #EDE7DB 50%, #F5F0E8 100%)', position: 'relative' }}>
      ...
      <div className="page-container">
        ...
        {/* Header */}
        <header className="site-header" style={{ animation: 'fadeIn 0.5s ease forwards' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ position: 'relative', flexShrink: 0 }}>
              <Image src="/logo.png" alt="Crown Coffee" width={42} height={42} style={{ borderRadius: '50%', display: 'block' }} />
              <div style={{ position: 'absolute', inset: -2, borderRadius: '50%', border: '1.5px solid rgba(200,146,42,0.3)', pointerEvents: 'none' }} />
            </div>
            <div>
              <div style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(13px, 3.5vw, 15px)', fontWeight: 700, color: 'var(--charcoal)', lineHeight: 1.2, letterSpacing: '-0.3px' }}>Crown Coffee</div>
              <div style={{ fontSize: 'clamp(8px, 2vw, 9px)', color: 'var(--gold)', letterSpacing: '2.5px', textTransform: 'uppercase', fontWeight: 600, marginTop: 1 }}>Multi Cuisine Café</div>
            </div>
          </div>

          <div style={{ display: 'flex', background: 'rgba(255,255,255,0.7)', backdropFilter: 'blur(12px)', border: '1px solid rgba(200,146,42,0.2)', borderRadius: 50, padding: 3, gap: 2, flexShrink: 0 }}>
            {['bn', 'en'].map(l => (
              <button key={l} onClick={() => setLang(l)} style={{
                padding: '6px 16px', border: 'none', borderRadius: 50,
        ...
        <div className="hero-section">
          <div style={{ display: 'inline-block', padding: '5px 18px', background: 'rgba(200,146,42,0.1)', borderRadius: 50, marginBottom: '1.25rem', border: '1px solid rgba(200,146,42,0.15)' }}>
          ...
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(22px, 6vw, 40px)', fontWeight: 700, color: 'var(--charcoal)', lineHeight: 1.25, marginBottom: '0.75rem', letterSpacing: '-0.5px' }}>{c.headline}</h1>
          <p style={{ color: 'var(--text-secondary)', fontSize: 'clamp(13px, 3.5vw, 15px)', maxWidth: 360, margin: '0 auto', lineHeight: 1.6 }}>{c.sub}</p>
        </div>

        {/* Form Card */}
        <div className="form-card" style={{ animation: 'fadeUp 0.7s ease 0.1s both forwards' }}>
            ...
            {[['kitchen', '🍳', c.kitchen, c.kitchenSub], ['front', '☕', c.front, c.frontSub]].map(([key, icon, label, sub]) => {
              ...
              return (
                <button key={key} onClick={() => selectDept(key)} style={{
                  ...
                  gap: 6, padding: '16px 16px 12px', textAlign: 'left',
                ...
                >
                  ...
                  <span style={{ fontSize: 'clamp(13px, 3.5vw, 15px)', fontWeight: 600, color: active ? 'var(--gold-deep)' : 'var(--charcoal)' }}>{label}</span>
                ...
                {POSITIONS[department].map(pos => {
                  ...
                  return (
                    <button key={pos} onClick={() => setPosition(pos)} style={{
                      ...
                      fontSize: 'clamp(12px, 3vw, 13px)', fontWeight: 500, cursor: 'pointer',
                      fontFamily: 'var(--font-body)', transition: 'all 0.25s',
                      letterSpacing: '0.2px', minHeight: 40,
                    }}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16, marginBottom: 28 }}>
            ...
            <InputField label={c.lblContact} placeholder={c.phContact} value={contact} onChange={setContact} type="tel" inputMode="tel" />
          ...
          <div
            ...
            style={{
              ...
              borderRadius: 16, padding: file ? '16px' : 'clamp(24px, 5vw, 36px) 16px',
          ...
          >
            ...
            {file ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <button onClick={e => { e.stopPropagation(); setFile(null); }} style={{ width: 34, height: 34, borderRadius: 50, border: '1px solid rgba(0,0,0,0.08)', background: 'var(--white)', cursor: 'pointer', fontSize: 14, color: 'var(--text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.2s', minHeight: 34 }}
              ...
              <>
                <div style={{ fontSize: 26, marginBottom: 8, animation: 'float 3s ease-in-out infinite', color: 'var(--text-muted)' }}>↑</div>
                <div style={{ color: 'var(--text-main)', fontSize: 'clamp(13px, 3.5vw, 14px)', fontWeight: 500, marginBottom: 6 }}>{c.uploadTitle}</div>
          ...
          {/* Error */}
          {errMsg && (
            <div style={{ padding: '12px 16px', borderRadius: 12, background: 'rgba(214,48,49,0.06)', border: '1px solid rgba(214,48,49,0.15)', color: 'var(--danger)', fontSize: 13, marginBottom: 20, fontFamily: 'var(--font-body)', animation: 'slideDown 0.3s ease forwards', display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <button onClick={handleSubmit} disabled={status === 'sending'} style={{
            ...
            fontSize: 'clamp(14px, 4vw, 16px)', fontWeight: 600, fontFamily: 'var(--font-body)',
        ...
        {/* Footer */}
        <div style={{ textAlign: 'center', marginTop: '2rem', animation: 'fadeIn 0.8s ease 0.3s both forwards' }}>
...
function InputField({ label, placeholder, value, onChange, type = 'text', inputMode }) {
    ...
    <div>
      ...
      <input type={type} inputMode={inputMode} value={value} placeholder={placeholder}
        ...
        style={{
          ...
          borderRadius: 12, color: 'var(--text-main)', fontSize: 16,
